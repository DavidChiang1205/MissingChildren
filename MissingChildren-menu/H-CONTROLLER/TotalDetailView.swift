...
class TotalDetailView: UIViewController,UINavigationControllerDelegate,MFMessageComposeViewControllerDelegate
{
    ...
    weak var TotalDataTableVC:TotalDataTableViewController!
    //記錄目前被點選的資料行
    var currentRow = 0
    //記錄目前處理中的學生資料
    var currentData = MissingPerson2()
    //記錄目前輸入元件的Y軸底緣位置
    ...
    override func viewDidLoad()
    {
        ...
        //哪一個儲存格被點選
        currentRow = TotalDataTableVC.tableView.indexPathForSelectedRow!.row
        //由上一頁的陣列紀錄當筆資料
        currentData = TotalDataTableVC.arrTable[currentRow]
        //將資料顯示在介面上:
        //顯示圖片：使用選擇性綁定語法
