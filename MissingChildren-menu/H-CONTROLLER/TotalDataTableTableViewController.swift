...
class TotalDataTableViewController: UITableViewController
{
    //滑動返回上一頁
    ...
    //記錄單一資料行
    var structRow = MissingPerson2()
    //宣告失蹤人口陣列，存放從資料庫查詢到的資料（此陣列即『離線資料集』）
    var arrTable = [MissingPerson2]()
    ...
    func getDataFromTable()
    {
        ...
        arrTable.removeAll()
        //準備查詢用的sql指令
        //let sql = "select no,name,picture,gender,age,nAge,time,place,reason,other from projectdata where gender = '\(filter_Gender)'and age = '\(filterAge)' ;"
        //let sql = "select no,name,picture,gender,age,nAge,time,place,reason,other from projectdata where age = '\(filterAge)';"
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?)
    {
        print("即將由導覽線換頁時")
        //由導覽線取得下一頁的執行實體
        let totaldetailVC = segue.destination as! TotalDetailView
        //通知下一頁目前本頁的執行實體所在位置
        totaldetailVC.TotalDataTableVC = self
